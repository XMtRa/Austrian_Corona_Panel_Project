...
In this extraordinary situation, people used media excessively to attain information, and especially social media were at an all time high [@statistaAverageDailyTime2021].
...
Several people reported that they were glued to their screens and could not pursue other relevant activities such as working, taking a break, or even child-care [@kleinDarklySoothingCompulsion2021].
As doomscrolling increased it became doubtful whether such a surge in social media use could still be considered useful and adaptive, or whether it created an additional hazard for users' mental health [@sandstromDoomscrollingCOVIDNews2021].
These concerns seem justified: A study with 6,233 people from Germany found that "[f]requency, duration and diversity of media exposure were positively associated with more symptoms of depression and unspecific and COVID-19 specific anxiety" [@bendauAssociationsCOVID19Related2021], which implies negative consequences for well-being.
...
Well-being can be further differentiated into subjective and psychological well-being [@dienerAdvancesOpenQuestions2018].
...
Whereas the first four levels are focused on the _channel_, the last two address the _type_ of communication.
...
I will distinguish (a) _reading_ COVID-19 related social media news (passive), _posting_ content regarding COVID (active), and _liking and sharing_ COVID-19 related posts (both active and passive).
...
Worth noting, this study is not about _general_ social media during times of COVID, but on social media use _focused_ on COVID-19 related news and interactions. 
...
On the other hand, the question of whether and how _general_ social media use affect well-being is well-researched.
...
And indeed, several if not most of the current meta-analyses find effect sizes below that threshold [@huangTimeSpentSocial2017; @meierComputerMediatedCommunicationSocial2020; @fergusonThisMetaanalysisScreen2021].

These overviews are well aligned with several individual new studies that employed advanced methods [@orbenSocialMediaEnduring2019; @przybylskiDoesTakingShort2021a; @schemerImpactInternetSocial2021; @kerestesAdolescentsOnlineSocial2020].
For example, @beyensSocialMediaUse2021 reported that although for some users (roughly one quarter) the effects were negative, for almost the same number of users they were positive, while for the majority the effects were neutral.
...
If a heavy user of COVID-19 related social media news suddenly _completely stops_ using social media, this should have a _noticeable_ impact on their overall well-being.
...
Transferred to our example, we would hence expect a SESOI of _b_ = 1 / 4 = .25.
...
We hence need to transform a 1-point change on a 7-point scale to its equivalent on an 11-point scale, which is 11 / 7 = `r 11/7`.
Hence, we now expect our effect to be at least as large as _b_ = .25 * `r 11/7` = `r 11/7*.25`.
...
Thus, only if we can control for _all_ potential varying third variables, can we correctly estimate causality without bias [@rohrerThinkingClearlyCorrelations2018].
...
However, in doing so not much is gained or lost because the effects of social media use would remain virtually the same [@klinePrinciplesPracticeStructural2016].[^mcelreath]

[^mcelreath]: But see @mcelreathYesterdayClass2021.
...
If a specific person changes their media diet, we need to measure how this affects _their_ well-being.
...
Similar approaches were implemented already by several other studies [@johannesNoEffectDifferent2021; @scharkowHowSocialNetwork2020] and they considered one of the best practices to analyze causality [@bellFixedRandomEffects2019].
...
The most relevant deviations are reported below, and a complete list of all changes can be found in the online companion website (https://xmtra.github.io/Austrian_Corona_Panel_Project/index.html).
...
In addition, because such large samples easily generate significant _p_-values even for very small effects, it helps that to test hypothesis this study uses a smallest effect size of interest-approach.
...
For example, if the 95% confidence interval lies completely within the null-region (e.g., _b_ = .15, [95% CI: .05, .25]), the hypothesis that the effect is only trivial is supported.
...
Reliability was also very high, omega = `r rel_aff_neg %>% my_round("std")`.
...
Regarding the effects of different _types_ of communication---that is, reading vs. sharing vs. posting---all within-person effects fell completely within the a-priori defined SESOIs (see Figure \@ref(fig:res-activity)).
...
```{r res-activity, fig.cap="The effects of various types of social media use on three indicators of well-being. Effects are controlled for a large number of covariates (see text). The SESOI was |0.30| for life satisfaction and |0.15| for affects. Hence, none of the reported effects are considered meaningful.", out.width = "\\textwidth", cache=F}
...
```{r res-channels, fig.cap="The effects of using various social media applications on three indicators of well-being. Effects are controlled for a large number of covariates (see text). The SESOI was |0.30| for life satisfaction and |0.15| for affects. Hence, none of the reported effects are considered meaningful.", out.width = "\\textwidth", cache=F}
knitr::include_graphics("figures/fig_results_channel.pdf")
```

Regarding the COVID-19 related use of social media _channels_, the results were very comparable (see Figure \@ref(fig:res-channels)).
...
Respondents who used Instagram more frequently than usual to attain COVID-19 related news reported slightly higher simultaneous levels of life satisfaction than usual (_b_ = `r dat_fig_results_channel %>% filter(dv == "Life satisfaction" & iv == "Instagram" & type == "within") %>% select(estimate) %>% my_round("b")` [95% CI `r dat_fig_results_channel %>% filter(dv == "Life satisfaction" & iv == "Instagram" & type == "within") %>% select(conf.low) %>% my_round("b")`, `r dat_fig_results_channel %>% filter(dv == "Life satisfaction" & iv == "Instagram" & type == "within") %>% select(conf.high) %>% my_round("b")`]).
Respondents who used Twitter more frequently than usual to attain COVID-19 related news reported somewhat lower simultaneous levels of life satisfaction than usual (_b_ = `r dat_fig_results_channel %>% filter(dv == "Life satisfaction" & iv == "Twitter" & type == "within") %>% select(estimate) %>% round(2)` [95% CI `r dat_fig_results_channel %>% filter(dv == "Life satisfaction" & iv == "Twitter" & type == "within") %>% select(conf.low) %>% round(2)`, `r dat_fig_results_channel %>% filter(dv == "Life satisfaction" & iv == "Twitter" & type == "within") %>% select(conf.high) %>% my_round("b")`]).
...
These studies have found that the effects of various types of social media use on several well-being indicators are small at best, often too small to matter [@meierComputerMediatedCommunicationSocial2020; @orbenTeenagersScreensSocial2020; @fergusonThisMetaanalysisScreen2021], which echoes the results obtained here.
...
Is it realistic that extremely changing only _one_ of these aspects (e.g., by completely stopping the use of social media) should already manifest in a detectable change in well-being?
Or would it make more sense to expect that if people regularly start doing say _two_ activities (e.g. regularly exercising _and_ establishing a reading habit) together should show perceivable improvements to well-being?
