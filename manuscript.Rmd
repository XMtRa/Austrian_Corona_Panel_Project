...
# 1devtools::install_github("https://github.com/crsh/papaja@devel")
...
It is often asked if social media use can become addictive. 
Users sometimes express this fear themselves [@yangCanWatchingOnline2021]. 
A recently published meta-analysis found that the two most prominent measures of addiction, the Bergen Facebook Addiction Scale and the Bergen Social Media Addiction Scale, have only small relations to well-being [@duradoniWellbeingSocialMedia2020]. 
That said, the general idea of labeling excessive social and new media use as addiction was criticized, arguing that social and new media represent new regular behaviors that should not be pathologized [@galerHowMuchToo2018; @vanrooijWeakScientificBasis2018].
...
This finding is aligned the Differential Susceptibility to Media Effects Model, which suggests that although there is substantial variation of media effects for individual users, the average effects reported in the literature are often small [@valkenburgDifferentialSusceptibilityMedia2013]. 
...
From a theoretical perspective, how could we explain whether well-being might be affected by COVID-19 related social media use in particular?
...
Although specific events such as marriage or salary can increase well-being, after some time it routinely returns to prior levels, which are mostly determined genetically [@sheldonStabilityHappinessTheories2014].
Only very specific very negative events such as unemployment, disability, or death can cause long-term decreases in well-being [@lucasAdaptationSetpointModel2007], and specific positive factors such as XXX to long-term increase [].
...
one could assume a _direct_ negative effect on well-being, and especially on positive or negative affect, which are more volatile and fluctuating.
...
A study with 2.057 respondents from Italy reported that during the pandemic virtual community and social connectedness potentially even increased [@guazziniSecondWaveAnalysis2022].
...
For more information on the analyses, a complete documentation of the models and results, see [companion website](https://XMtRA.github.io/Austrian_Corona_Panel_Project).
...
COVID-19 related social media use focused on communication types was measured with the three dimensions of (a) reading, (b) liking and sharing, and (c) posting from @wagnerAUTNESOnlinePanel2018, adapted for the context of this study.
...
First, when looking at the variables from a descriptive perspective (Figure 1), we see that all well-being measures did not change substantially across the different waves of data collection.
