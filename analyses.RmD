...
# define packages
packages <- c("corrplot", "devtools", "GGally", "ggplot2", "gridExtra", "haven", "kableExtra", "knitr", 
              "lavaan", "lme4", "magrittr", "mice", "MVN", "PerFit", "psych", "quanteda.textstats",
...
```{r}
make_graph <- function(model, title, ll, ul, line = FALSE, labels = FALSE){
  ...
  graph <- 
    ggplot(dat, aes(wave, value)) +
    theme_bw() +
    theme(text = element_text(size = 8)) + 
    geom_bar(data = dat, aes(wave, value), stat = "identity") +
...
make_graph_res <- function(data){
  ...
  geom_vline(data=filter(data, dv=="Life satisfaction"), aes(xintercept=.3), colour="darkgrey", linetype = "dashed") + 
    geom_vline(data=filter(data, dv=="Life satisfaction"), aes(xintercept=-.3), colour="darkgrey", linetype = "dashed") + 
  geom_vline(data=filter(data, dv!="Life satisfaction"), aes(xintercept=.15), colour="darkgrey", linetype = "dashed") + 
    geom_vline(data=filter(data, dv!="Life satisfaction"), aes(xintercept=-.15), colour="darkgrey", linetype = "dashed") + 
  theme(
...
# d_raw <- read_spss("data/10094_da_de_v4_0.zsav")
...
```{r}
# recode such that higher values imply more strenght / align with wording
d_long %<>% 
  mutate_at(vars(med_txt_kro:med_vid_pri, health, sat_dem), 
...
Create mean values for each person, and unique changes per wave

```{r}
d_long %<>% 
  group_by(id) %>% 
  mutate_at(vars(health:loc_cntrl_int_4), funs(b = mean(., na.rm = TRUE), w = . - mean(., na.rm = TRUE))) %>% 
  ungroup()
```
...
fig_life_sat <- make_graph(model_life_sat, "Life satisfaction", ll = 0, ul = 10)
fig_life_sat
```
...
fig_aff_pos <- make_graph(model_aff_pos, "Positive affect", ll = 1, ul = 5)
fig_aff_pos
```
...
fig_aff_neg <- make_graph(model_aff_neg, "Negative affect", ll = 1, ul = 5)
fig_aff_neg
```
...
fig_soc_med_read <- make_graph(model_soc_med_read, "Social Media Reading", ll = 1, ul = 5)
fig_soc_med_read
```

## Social media liking & sharing

```{r}
model_soc_med_like_share <- lmer(soc_med_like_share ~ (1 | id) + (1 | wave), d_long)
fig_soc_med_like_share <- make_graph(model_soc_med_like_share, "Soc. Media Like & Share", ll = 1, ul = 5)
fig_soc_med_like_share
```
...
fig_soc_med_post <- make_graph(model_soc_med_post, "Social Media Posting", ll = 1, ul = 5)
fig_soc_med_post
```

## figure

Save everything as a figure

```{r}
fig_desc <- grid.arrange(fig_life_sat, fig_aff_pos, fig_aff_neg, 
                         fig_soc_med_read, fig_soc_med_like_share, fig_soc_med_post,
                         nrow = 2)
...
```{r}
model_1 <- lmerTest::lmer(life_sat ~ (1 | id) + soc_med_read + soc_med_like_share + soc_med_post + soc_med_fb + soc_med_ig + soc_med_wa + soc_med_yt + soc_med_tw,
                data = d_long)
summary(model_1)
```

Now with diff. between within and between effects.

```{r}
model_2 <- lmerTest::lmer(life_sat ~ (1 | id) + (1 | wave) + soc_med_read_b + soc_med_read_w + soc_med_like_share_b + soc_med_like_share_w + soc_med_post_b + soc_med_post_w + soc_med_fb_b + soc_med_fb_w + soc_med_ig_b + soc_med_ig_w + soc_med_wa_b + soc_med_wa_w + soc_med_yt_b + soc_med_yt_w + soc_med_tw_b + soc_med_tw_w,
                data = d_long)
summary(model_2)
```
...
```{r}
model_life_sat_lmer <- lmerTest::lmer(life_sat ~ (1 | id) + (1 | wave) + soc_med_read_b + soc_med_read_w + soc_med_like_share_b + soc_med_like_share_w + soc_med_post_b + soc_med_post_w + soc_med_fb_b + soc_med_fb_w + soc_med_ig_b + soc_med_ig_w + soc_med_wa_b + soc_med_wa_w + soc_med_yt_b + soc_med_yt_w + soc_med_tw_b + soc_med_tw_w + age + male + edu + born_aus + born_aus_prnts + res_vienna + edu + acc_bal + acc_gar + employment_fac
...
```{r}
model_aff_pos_lmer <- lmerTest::lmer(aff_pos_m ~ (1 | id) + (1 | wave) + soc_med_read_b + soc_med_read_w  + soc_med_like_share_b + soc_med_like_share_w + soc_med_post_b + soc_med_post_w + soc_med_fb_b + soc_med_fb_w + soc_med_ig_b + soc_med_ig_w + soc_med_wa_b + soc_med_wa_w + soc_med_yt_b + soc_med_yt_w + soc_med_tw_b + soc_med_tw_w + age + male + edu + born_aus + born_aus_prnts + res_vienna + edu + acc_bal + acc_gar + employment_fac
# + health_b + act_wrk_b + act_spo_b + act_frn_b + act_sho_b + act_pet_b + sat_dem_b + hh_income_b + med_txt_kro_b + med_txt_sta_b + med_txt_pre_b + med_txt_oes_b + med_txt_kur_b + med_txt_slz_b + med_txt_son_b + med_vid_orf_b + med_vid_pri_b + risk_prop_b + loc_cntrl_int_1_b + loc_cntrl_int_2_b + loc_cntrl_int_3_b + loc_cntrl_int_4_b + health_w + act_wrk_w + act_spo_w + act_frn_w + act_sho_w + act_pet_w + sat_dem_w + hh_income_w + med_txt_kro_w + med_txt_sta_w + med_txt_pre_w + med_txt_oes_w + med_txt_kur_w + med_txt_slz_w + med_txt_son_w + med_vid_orf_w + med_vid_pri_w + risk_prop_w + loc_cntrl_int_1_w + loc_cntrl_int_2_w + loc_cntrl_int_3_w + loc_cntrl_int_4_w
,
                data = d_long)
...
```{r}
model_aff_neg_lmer <- lmerTest::lmer(aff_neg_m ~ (1 | id) + (1 | wave) + soc_med_read_b + soc_med_read_w + soc_med_like_share_b + soc_med_like_share_w + soc_med_post_b + soc_med_post_w + soc_med_fb_b + soc_med_fb_w + soc_med_ig_b + soc_med_ig_w + soc_med_wa_b + soc_med_wa_w + soc_med_yt_b + soc_med_yt_w + soc_med_tw_b + soc_med_tw_w + age + male + edu + born_aus + born_aus_prnts + res_vienna + edu + acc_bal + acc_gar + employment_fac
# + health_b + act_wrk_b + act_spo_b + act_frn_b + act_sho_b + act_pet_b + sat_dem_b + hh_income_b + med_txt_kro_b + med_txt_sta_b + med_txt_pre_b + med_txt_oes_b + med_txt_kur_b + med_txt_slz_b + med_txt_son_b + med_vid_orf_b + med_vid_pri_b + risk_prop_b + loc_cntrl_int_1_b + loc_cntrl_int_2_b + loc_cntrl_int_3_b + loc_cntrl_int_4_b + health_w + act_wrk_w + act_spo_w + act_frn_w + act_sho_w + act_pet_w + sat_dem_w + hh_income_w + med_txt_kro_w + med_txt_sta_w + med_txt_pre_w + med_txt_oes_w + med_txt_kur_w + med_txt_slz_w + med_txt_son_w + med_vid_orf_w + med_vid_pri_w + risk_prop_w + loc_cntrl_int_1_w + loc_cntrl_int_2_w + loc_cntrl_int_3_w + loc_cntrl_int_4_w
,
                data = d_long)
...
dat_fig_results <- 
  ...
  mutate(
    type = ifelse(grepl(".*_b.*", .$term), "between", "within"),
    iv = gsub("_(w|b)\\>", "", .$term)
         ) %>%
  mutate(
    type = factor(.$type, levels = c("within", "between")),
    dv = factor(.$dv, levels = c("life_sat", "aff_pos", "aff_neg"),
...
dat_fig_results_activity <- dat_fig_results %>%
  ...
  mutate(
    iv = factor(.$iv, levels = c("soc_med_post", "soc_med_like_share", "soc_med_read"), labels = c("Posting", "Liking & Sharing", "Reading"))
  )
