...
Positive affect was measured as a scale. We hence first inspect factorial validty using CFA.

```{r}
model <- "
aff_pos =~ a1*aff_pos_1 + a2*aff_pos_2 + a3*aff_pos_3
"
cfa_aff_pos <- cfa(model, d_long, group = "wave")
...
Negative affect was measured as a scale. We hence first inspect factorial validty using CFA.

```{r}
model <- "
aff_neg =~ a1*aff_neg_1 + a2*aff_neg_2 + a3*aff_neg_3 + a4*aff_neg_4 + a5*aff_neg_5 + a6*aff_neg_6
"
cfa_aff_neg <- cfa(model, d_long, group = "wave")
...
"
cfa_loc_cntrl_int <- cfa(model, filter(d_long, wave != 20), group = "wave")
